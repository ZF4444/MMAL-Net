...
class CUB():
    ...
    def __getitem__(self, index):
        if self.is_train:
            ...
            # img = transforms.RandomRotation(45, resample=Image.BILINEAR, expand=True, center=None)(img)
...
class Stanford_Dogs():
    def __init__(self, input_size, root, is_train=True, data_len=None):
        ...
        train_label_file = open(os.path.join(self.root,'lists', 'train_label.txt'))
        test_label_file = open(os.path.join(self.root, 'lists', 'test_label.txt'))
        train_img_label = []
        test_img_label = []
        for line in train_label_file:
